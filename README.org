...
- The Zettelkasten is simply a directory containing org-documents, where each document corresponds to a topic.
- Links between topics are created automagically.  No need to create and manage links manually.  No need to update them when a linked document changes.  Automatic links were the motivation to write this package.
- No database needed.  MWK uses silversearcher (fast!) to see what’s the current state of affairs in your Zettelkasten.
